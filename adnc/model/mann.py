...
class MANN():
    def __init__(self,
        ...
        self.mask = tf.compat.v1.placeholder(self.dtype,
                                             [None, self.batch_size],
                                             name='mask')
        self.target = tf.compat.v1.placeholder(
            self.dtype, [None, self.batch_size, self.output_size], name='y')
        ...
        if self.input_embedding:
            ...
            if 'tmp_dir' in self.input_embedding:
                ...
                tmp_dir = "data_tmp"
            glove = WordEmbedding(embedding_size,
                                  word_idx_dict=word_idx_dict,
                                  initialization='glove',
                                  tmp_dir=tmp_dir)

            self._data = tf.compat.v1.placeholder(tf.int64,
                                                  [None, self.batch_size],
                                                  name='x')
            self.data = glove.embed(self._data)
        else:
            self.data = tf.compat.v1.placeholder(
                tf.float32, [None, self.batch_size, self.input_size], name='x')

        if self.architecture in ['uni', 'unidirectional']:
            unweighted_outputs, states = self.unidirectional(
                self.data,
                self.controller_config,
                self.memory_unit_config,
                reuse=self.reuse)
        elif self.architecture in ['bi', 'bidirectional']:
            unweighted_outputs, states = self.bidirectional(
                self.data,
                self.controller_config,
                self.memory_unit_config,
                reuse=self.reuse)
            ...
            raise UserWarning(
                "Unknown architecture, use unidirectional or bidirectional")

        if self.analyse:
            with tf.device('/cpu:0'):
                if self.architecture in ['uni', 'unidirectional']:
                    analyse_outputs, analyse_states = self.unidirectional(
                        self.data,
                        self.controller_config,
                        self.memory_unit_config,
                        analyse=True,
                    ...
                    self.analyse = (analyse_outputs, analyse_signals,
                                    analyse_states)
                elif self.architecture in ['bi', 'bidirectional']:
                    analyse_outputs, analyse_states = self.bidirectional(
                        self.data,
                        self.controller_config,
                        self.memory_unit_config,
                        analyse=True,
    ...
    def _output_layer(self, outputs):
        ...
        with tf.compat.v1.variable_scope("output_layer"):
            ...
            weights_concat = tf.compat.v1.get_variable(
                "weights_concat", (output_size, self.output_size),
                initializer=tf.compat.v1.keras.initializers.VarianceScaling(
                    scale=1.0,
                    mode="fan_avg",
                    distribution="uniform",
                    seed=self.seed),
                collections=['mann', tf.compat.v1.GraphKeys.GLOBAL_VARIABLES],
                dtype=self.dtype)
            bias_merge = tf.compat.v1.get_variable(
                "bias_merge", (self.output_size, ),
        ...
        predictions = tf.reshape(predictions_flat,
                                 [-1, self.batch_size, self.output_size])
        weighted_outputs = tf.reshape(output_flat,
                                      [-1, self.batch_size, self.output_size])
    ...
    def get_loss(self, prediction):
        ...
        if self.loss_function == 'cross_entropy':
            if self.output_mask:
                cost = tf.reduce_sum(
                    input_tensor=-1 * self.target *
                    tf.math.log(tf.clip_by_value(prediction, 1e-12, 10.0)) -
                    (1 - self.target) *
                    tf.math.log(tf.clip_by_value(1 - prediction, 1e-12, 10.0)),
                    axis=2)
                cost *= self.mask
                loss = tf.reduce_sum(input_tensor=cost) / tf.reduce_sum(
                    input_tensor=self.mask)
            else:
                loss = tf.reduce_mean(
                    input_tensor=-1 * self.target *
                    tf.math.log(tf.clip_by_value(prediction, 1e-12, 10.0)) -
            ...
            mse = tf.reduce_mean(input_tensor=mse, axis=2)
            ...
            if self.output_mask:
                ...
                loss = tf.reduce_mean(input_tensor=mse)
        else:
            raise UserWarning(
                "Unknown loss function, use cross_entropy or mse")
        return loss
    ...
    def unidirectional(self,
        ...
        with tf.name_scope("controller"):
            controller_list = get_rnn_cell_list(controller_config,
                                                name='controller',
                                                seed=self.seed,
                                                dtype=self.dtype)
        ...
        if controller_config['connect'] == 'sparse':
            ...
            mu_cell = get_memory_unit(memory_input_size,
                                      memory_unit_config,
                                      'memory_unit',
                                      analyse=analyse,
                                      reuse=reuse)
        ...
        output_init = tf.zeros([batch_size, cell.output_size],
                               dtype=self.dtype)

        if analyse:
            output_init = (output_init,
                           mu_cell.analyse_state(batch_size, dtype=self.dtype))
    ...
    def bidirectional(self,
        ...
        with tf.name_scope("controller"):
            list_fw = get_rnn_cell_list(controller_config,
                                        name='con_fw',
                                        seed=self.seed,
                                        dtype=self.dtype)
            list_bw = get_rnn_cell_list(controller_config,
                                        name='con_bw',
                                        seed=self.seed,
                                        dtype=self.dtype)
        if controller_config['connect'] == 'sparse':
        ...
        cell_mu = get_memory_unit(memory_input_size,
                                  memory_unit_config,
                                  'memory_unit',
                                  analyse=analyse,
                                  reuse=reuse)
        ...
        with vs.variable_scope("bw") as bw_scope:
            ...
            output_bw, output_state_bw = tf.compat.v1.nn.dynamic_rnn(
                cell=cell_bw,
        ...
        batch_size = inputs.get_shape()[1].value
        cell_fw_init_states = cell_fw.get_initial_state(batch_size, dtype=self.dtype)
        cell_mu_init_states = cell_mu.get_initial_state(batch_size, dtype=self.dtype)
        output_init = tf.zeros([batch_size, cell_mu.output_size],
                               dtype=self.dtype)

        if analyse:
            output_init = (output_init,
                           cell_mu.analyse_state(batch_size, dtype=self.dtype))
        ...
        with vs.variable_scope("fw") as fw_scope:
            ...
            def step(pre_states, coupled_inputs):
                ...
                output_fw, states_fw = cell_fw(controller_inputs,
                                               pre_states_fw)
    ...
    def controller_trainable_variables(self):
        return tf.compat.v1.get_collection('recurrent_unit')
    ...
    def mann_trainable_variables(self):
        return tf.compat.v1.get_collection('mann')
    ...
    def memory_unit_parameter_amount(self):
        return self.count_parameter_amount(
            self.memory_unit_trainable_variables)
